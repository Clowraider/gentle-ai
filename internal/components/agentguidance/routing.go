...
func RenderRouting(agent model.AgentID) (string, error) {
	...
	output.WriteString("  - **Design-minimality pass:** between exploration and implementation, run a design-minimality pass before composing the writer brief. Apply the minimality ladder (YAGNI -> reuse -> stdlib -> platform -> dependency -> minimum new code), estimate line footprint, record deliberate deferrals, and pass the minimal plan as the writer brief.\n")
