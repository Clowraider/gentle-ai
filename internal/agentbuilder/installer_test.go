...
func TestInstall_InvalidAgentNameReturnsError(t *testing.T) {
	invalidNames := []string{"", ".", "..", "../agent", "/absolute/agent"}
	for _, name := range invalidNames {
		t.Run(name, func(t *testing.T) {
			_, err := Install(makeAgent(name, "# Agent\n"), []AdapterInfo{}, "")
			if err == nil {
				t.Fatalf("Install(%q) error = nil, want validation error", name)
			}
		})
	}
}

func TestUninstall_RemovesSkillFilesFromAllAdapters(t *testing.T) {
	dir1 := t.TempDir()
	dir2 := t.TempDir()
	adapters := []AdapterInfo{
		{AgentID: model.AgentClaudeCode, SkillsDir: dir1},
	...
	removed, err := Uninstall(agent.Name, adapters)
	if err != nil {
		t.Fatalf("Uninstall() error = %v", err)
	}
	if len(removed) != len(adapters) {
		t.Fatalf("len(removed) = %d, want %d", len(removed), len(adapters))
	}
	for _, adapter := range adapters {
		skillFile := filepath.Join(adapter.SkillsDir, agent.Name, "SKILL.md")
		if _, err := os.Stat(skillFile); !os.IsNotExist(err) {
			t.Errorf("skill file %q still exists after uninstall", skillFile)
		}
	}
}

func TestUninstall_MissingSkillIsIgnored(t *testing.T) {
	removed, err := Uninstall("missing-agent", []AdapterInfo{
		{AgentID: model.AgentClaudeCode, SkillsDir: t.TempDir()},
	})
	if err != nil {
		t.Fatalf("Uninstall() error = %v", err)
	}
...
func TestUninstall_InvalidAgentNameReturnsError(t *testing.T) {
	invalidNames := []string{"", ".", "..", "../agent", "/absolute/agent"}
	for _, name := range invalidNames {
		t.Run(name, func(t *testing.T) {
			_, err := Uninstall(name, []AdapterInfo{})
			if err == nil {
				t.Fatalf("Uninstall(%q) error = nil, want validation error", name)
			}
		})
	}
}
